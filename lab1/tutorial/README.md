...
此外，我们还需要安装`nasm`这个汇编代码编译器。但是，在ubuntu 18.04下，我们使用`apt`安装的是版本`2.13`的。后面我们可以看到，我们真正需要的版本是`2.15`的。因此，我们需要参考`/tutorial/tools/debug_with_gdb_and_qemu`安装`2.15`版本的`nasm`。
...
从[Busybox官网](https://busybox.net/downloads/)下载Busybox到`~/lab1`，然后解压。
