...
## Assignment 1：计算机启动过程：MBR（Master Boot Record）

> 注意，assignment 1的寄存器请使用16位的寄存器。

### 1.1

复现下文中的Example：Hello World。说说你是怎么做的，并将结果截图。

### 1.2

请修改/tutorial/mbr/mbr.asm，使得MBR被加载到0x7C00后在$(12,12)$处开始输出你的学号。注意，你的学号显示的前景色和背景色必须和教程中不同。说说你是怎么做的，并将结果截图。

## Assignment 2：利用中断实现交互
...
2.1和2.2使用的都是实模式中断`int 10h`，由于功能号不同，执行的结果也就不同。在`int 10h`中断的资料 https://zh.wikipedia.org/wiki/INT_10H 中，其只给出10h中断下各个功能号的用途，并未给出实际的用法。因此，同学们可能一开始会感觉不知所云，教程下面给出同学们完成Assignment2需要用到的功能号。

> 注：int 10h，h表示十六进制，等价于int 0x10
...
一般地，中断的调用方式如下。[tutorial/test1.asm](./test1.asm)给出了一个简单的例子，同学们可以运行体验一下。
...
> + `assignment`目录下包含`makefile`文件。在多个文件的编译时，我们如果分别对每一个文件进行编译，如：g++ xxxx。那我们每一次编译都需要输入诸多类似命令，makefile则是将这些命令集成到一起，然后在命令行输入：make，即可执行这些编译过程。
> + `assignment`目录下的head.include和end.include为固定文件，不需要修改。
...
**在assignment/student.asm中已经给出小部分代码供参考。修改test.cpp中a1使之小于12，然后在assignment目录下输入`make run`编译运行。**
...
经过以上四个作业，同学们应该对计算机启动过程、中断机制的调用、汇编语言的编写有了初步的认知。请将Assignment1、2、4整合到同一个程序中，实现一个简单的Operating System。具体如下：

1. 计算机开机后，在实模式中断下输出你的学号。字体需居中显示。
2. 在你的学号下面输出“Press any key to start”。使用任意按键进入到下一个界面。
3. 在下一个界面，你需要将Assignment4的内容显示出来。

要求：程序能正常运行。在完成基础功能的情况下，字体、布局需美观。

#### 参考资料：/tutorial/热身小实验。小实验的代码中包含了各种中断使用的用例，以及函数的声明，是一份很好的学习资料。
